...
int main(int argc, char* argv[]){
    ...
    std::string filename = "darkside_"+std::to_string(omp_get_max_threads())+"th_"+
           std::to_string(n_runs)+"r_"+std::to_string(d)+"d_"+std::to_string(max_nr_tables)+"t.csv";
    // std::string filename = "testfile.csv";

    std::ofstream myfile;
    myfile.open (filename, std::ios::app);
    myfile << "# Executing on "<< max_threads <<  " threads; testing the dark side\n";
    ...
    for(int thr=24; thr<=37; ++thr) {
        for(int run=0; run<n_runs; ++run) {
            ...
            sort_times[run] = time_elapsed.count();
        }
